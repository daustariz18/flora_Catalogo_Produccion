...
### tema
Tema visual (color, fuente y tamaño) del catálogo web de una empresa. Relación 1-a-1 con `empresa`.

| columna | tipo | null | descripción |
|---|---|---|---|
| id_tema | bigint | NO | PK, autoincremental |
| empresa_id | bigint | NO | FK → empresa. UNIQUE |
| color_primario | varchar(20) | NO | Color de marca (botones, acentos). DEFAULT `#d94b8a` |
| color_secundario | varchar(20) | NO | Tono derivado para degradados/hover. DEFAULT `#8f2e56` |
| color_fondo | varchar(20) | NO | Fondo general de la página. DEFAULT `#fffdf9` |
| color_fondo_suave | varchar(20) | NO | Fondo suave (paneles, círculos). DEFAULT `#fff6f7` |
| color_texto | varchar(20) | NO | Texto principal. DEFAULT `#241923` |
| color_texto_suave | varchar(20) | NO | Texto secundario/atenuado. DEFAULT `#6d5b68` |
| color_borde | varchar(20) | NO | Bordes y separadores. DEFAULT `#efd9e1` |
| fuente_familia | varchar(150) | NO | CSS `font-family`. DEFAULT `Segoe UI, Tahoma, Geneva, Verdana, sans-serif` |
| fuente_tamano_base | varchar(10) | NO | CSS `font-size` raíz. DEFAULT `16px` |
| activo | boolean | NO | |
| created_at | timestamp | NO | |
| updated_at | timestamp | SÍ | |

**Uso actual:** Todas las empresas comparten el mismo tema por defecto (backfill automático al crear la tabla).
Se expone en `GET /api/public/{slug}/empresa` y en el bloque `empresa` de `GET /api/public/{slug}/catalogo`.
Se edita vía `GET/PUT /tema` (autenticado, admin de la empresa en sesión) para que cada cliente pueda cambiar su color a conveniencia. Si una empresa no tiene fila propia, las queries públicas devuelven el tema por defecto vía `COALESCE`.
